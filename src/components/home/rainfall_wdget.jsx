    ...
    <>
      ...
      <div className="relative text-xl min-w-[30vw] max-w-[40rem] bg-[rgba(0,0,0,.8)] rounded-xl h-max mx-0 my-0 flex flex-col p-1 py-1 shadow-lg z-10">
        <div className="relative flex justify-center ">
          ...
          <div className="flex justify-evenly text-xs text-white font-bold flex-col text-center">
            <span className="text-white text-xs font-bold mx-1 my-2">
              {data.station.name}
            </span>
          </div>
        </div>
...
const dailyPredictionOptions2 = {
  ...
  legend: {
    position: "bottom",
    alignment: "center",
    textStyle: { color: "#fff", fontName: "Nunito Sans", fontSize: 10 },
  },
...
const rainfallBarChartData = (data) => [
  ...
  ...data.hrly_data.map((item) => [item.hour, item.total_rainfall]),
];
...
const dailyPredictionChartData = (data) => {
  const combinedData = data.daily_data.map((item, index) => {
    ...
    const color =
      index >= data.daily_data.length - 3 ? getColor(predicted) : null; // Apply color to the last three
    ...
    return [
      ...
      index >= data.daily_data.length - 3
        ? `<div style="padding:0px;">${dateLabel}<b>Predicted:</b>${predicted.toFixed(
            2
          )}mm</div>`
        : null, // Tooltip for the last three
    ];
  });
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="absolute left-4 bottom-2 text-white text-sm"
      >
        <ArrowBigLeft />
      </button>
      <button
        onClick={handleNext}
        disabled={range.end >= totalDataPoints}
        className="absolute right-8 bottom-2 text-white text-sm"
      >
        <ArrowBigRight />
      </button>
    </div>
